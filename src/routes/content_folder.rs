...
pub async fn show(
    ...
    let sub_content_folders: Vec<content_folder::Model> = content_folder_operator
        .list_by_parent_and_category(
            Some(current_content_folder.id),
            current_content_folder.category_id,
        )
        .await
        .context(ContentFolderSnafu)?;
...
pub async fn create(
    ...
    let parent_path = match form.parent_id {
        Some(parent_id) => {
    ...
    if form.name.contains("/") {
        ...
        let uri = format!("/folders/{}{}", category.name, parent_path);

        return Ok((jar, Redirect::to(uri.as_str()).into_response()));
    }

    // build final path with parent_path and path of form and normalized it with Utf8PathBuf
    form.path = Utf8PathBuf::from(format!("{}/{}", parent_path, form.name))
        .components()
        .collect::<Utf8PathBuf>()
        .into_string();
